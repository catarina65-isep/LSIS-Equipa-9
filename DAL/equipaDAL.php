...
class EquipaDAL {
    ...
    public function adicionarMembroEquipa($equipaId, $utilizadorId, $coordenador = false) {
        try {
            ...
            // A validação de coordenador ativo é feita na camada BLL
            // Aqui apenas adicionamos o membro conforme solicitado
    ...
    public function obterTodasEquipas() {
        // Primeiro, busca todas as equipes ativas com contagem de membros
        $sql = "SELECT e.*, 
                       (SELECT COUNT(*) FROM equipa_membros em WHERE em.equipa_id = e.id_equipa AND em.ativo = 1) as total_membros
                FROM equipa e 
                WHERE e.ativo = 1 
                ORDER BY e.nome";
        $stmt = $this->pdo->query($sql);
        ...
        foreach ($equipas as &$equipa) {
            // Primeiro tenta buscar o coordenador diretamente pelo id_coordenador da tabela equipa
            if (!empty($equipa['id_coordenador'])) {
                $sqlCoordenador = "SELECT 
                                ...
                                JOIN utilizador u ON co.id_utilizador = u.id_utilizador
                                LEFT JOIN colaborador c ON u.id_utilizador = c.id_utilizador
                                WHERE co.id_coordenador = :id_coordenador";
                
                $stmtCoordenador = $this->pdo->prepare($sqlCoordenador);
                $stmtCoordenador->execute([':id_coordenador' => $equipa['id_coordenador']]);
                $coordenador = $stmtCoordenador->fetch(PDO::FETCH_ASSOC);
            }
            
            // Se não encontrou pelo id_coordenador, tenta pelo id_utilizador (backward compatibility)
            if (empty($coordenador) && !empty($equipa['coordenador_id'])) {
                $sqlCoordenador = "SELECT 
                                    u.id_utilizador,
                                    COALESCE(CONCAT(c.nome, ' ', c.apelido), u.username) as nome,
                                    u.email,
                ...
                $stmtCoordenador = $this->pdo->prepare($sqlCoordenador);
                $stmtCoordenador->execute([':coordenador_id' => $equipa['coordenador_id']]);
                $coordenador = $stmtCoordenador->fetch(PDO::FETCH_ASSOC);
            }
